...
def draw_links():
  ...
  for (src, dst) in graph.edges:
    canvas.create_line(nodes[src]['x'], nodes[src]['y'], nodes[dst]['x'], nodes[dst]['y'], fill="black")
...
def draw_nodes():
  ...
  for node, val in nodes.items():
    ...
    if val['type'] == 'Switch':
      box_size = 40
      canvas.create_rectangle(val['x']-box_size/2, val['y']-box_size/2, val['x']+box_size/2, val['y']+box_size/2, fill=color)
      canvas.create_text(val['x'], val['y']-box_size/2-15, text=f'{val["name"]}')

    else:
      canvas.create_oval(val['x']-10, val['y']-10, val['x']+10, val['y']+10, fill=color)
      canvas.create_text(val['x']-15, val['y']+35, text=f'{val["name"]}')
...
def add_node(x: float, y: float, node_type : str):
  ...
  if node_type == 'Switch':
    graph.nodes[next_node_num].update({'x': x, 'y': y, 'type': node_type, 'name': f'net_{next_node_num}'})
  else:
    graph.nodes[next_node_num].update({'x': x, 'y': y, 'type': node_type, 'name': f'n{next_node_num}'})
...
def toolbar(root):
    ...
    root.bind('<Control-1>', lambda e: print("Ctrl+1"))
    ...
    combo = ttk.Combobox(btn_bar, textvariable=nodevar, )
...
def canvas_drag(event):
    ...
    if active_tool == "Link":
        if selected_node != 0:
            draw_nodes()
            canvas.create_line(graph.nodes[selected_node]['x'], graph.nodes[selected_node]['y'], lastx, lasty, fill="black")
...
def on_menu_open_file():
  ...
  current_filename = filedialog.askopenfilename(defaultextension=".graphml", filetypes=[("GraphML files", "*.graphml"), ("All files", "*.*")])
...
def on_menu_save_as_file():
  ...
  global current_filename
  global root
  current_filename = filedialog.asksaveasfilename(defaultextension=".graphml", filetypes=[("GraphML files", "*.graphml"), ("All files", "*.*")])
  if current_filename != None and current_filename != '':
    nx.write_graphml(graph, current_filename)
    root.title(f"NetEdit - {current_filename}")
...
def on_menu_about():
  print("About")
  messagebox.showinfo("About", "NetEdit v0.1\n\n(c) 2022, Lars Baumgaertner\nAll rights reserved.")
...
canvas = Canvas(canvas_frame, 
           width=canvas_width,
           height=canvas_height, bg='white', scrollregion=(0, 0, 2000, 2000), yscrollcommand=v.set, xscrollcommand=h.set)
...
status_nodes = ttk.Label(statusframe, textvariable=statusNodesVar, relief=SUNKEN, anchor=E, width=10)
...
status_links = ttk.Label(statusframe, textvariable=statusLinksVar, relief=SUNKEN, anchor=E, width=10)
status_links.pack(padx=10, side=LEFT)

statusframe.grid(row=3, column=0, columnspan=4, sticky=(W, E, S), pady=10)

btn_bar.grid(row=0, column=0, columnspan=2, padx=10, pady=10, sticky=(N, E, W))
